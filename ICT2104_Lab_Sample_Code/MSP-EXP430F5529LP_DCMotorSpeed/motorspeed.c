...
char buffer[5];
int snr1=0;        	//variable stores interrupt edge
...
unsigned short adc_value;		// power value from ADC

// monitor speed
unsigned char holes = 5;
unsigned short counts = 0;
unsigned short cycles = 0;
unsigned short speed_num = 0;
unsigned short speed_dec = 0;
unsigned char refresh_rate = 1;
...
//    P1IES |= BIT2 + BIT3;         //Interrupt on Hi->Lo transition
	...
	// monitor speed
	P1SEL = BIT2;                   				// Set P1.2 to TA0.1
	TA0CCTL0 = CCIE; 								// CCR0 interrupt enabled
	TA0CCTL1 |= CM_1 + SCS + CCIS_0 + CAP + CCIE; 	// Rising edge + CCI0A (P2.0) + Capture Mode + Interrupt
	...
	for(;;)
	{
		...
		if (adc_value<=5) adc_value=5;			// just to set min value
		if (adc_value>=4090) adc_value=4090;			// just to set max value

		if(TA0CCR3 < (adc_value-5))			// tol of +/-5
			TA0CCR3+=3;
		else if (TA0CCR3 > (adc_value+5))
			TA0CCR3-=3;
	    ...
	    refresh_rate = adc_value/100;	// approx constant refresh rate
	    if (refresh_rate < 5)
	    	refresh_rate = 5;	// min after 5 rotations

	    if((counts/holes)>=refresh_rate)	// compute speed
	    {
			speed_num = (counts*8)/(cycles*holes);	// 8*4096 = 32768
			gotoXy(8,0);
//				lcdData(speed_num/10 + 0x30);
			lcdData(speed_num%10 + 0x30);
			lcdData('.');
			speed_dec = ((counts*80)/(cycles*holes))%10;
			lcdData(speed_dec + 0x30);
			counts=cycles=0;
		}
	}
}
...
//	static unsigned char count=0;
    ...
    //check interrupt flags to find which interrupt was triggered
    //Check for BIT2 interrupt
    if ((P1IFG & BIT2)==BIT2)		// falling or raising edge
    {
        if ((P1IN & BIT2)==BIT2) {snr1=1;} else {snr1=0;}  //read pin state
    ...
    if ((P1IFG & BIT3)==BIT3)		// falling or raising edge
    {
        if ((P1IN & BIT2)==BIT2) {snr1=1;} else {snr1=0;}  	//read pin state
        if ((P1IN & BIT3)==BIT3) {b=1;} else {b=0;}     //read pin state
        cval = (snr1<<0) + (b<<1);
